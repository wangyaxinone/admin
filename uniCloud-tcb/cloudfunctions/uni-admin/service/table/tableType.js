...
module.exports = class MenuService extends Service {
	async add(data) {
		...
		return await this.db.collection('opendb-admin-tableType').add(data);
	}
	async update(data) {
		const {
			_id
		...
		data.update_date = getServerDate();
		data.operator = this.ctx.auth.uid;
		return await this.db.collection('opendb-admin-tableType').doc(_id).update(data);
	}
	async remove(_ids) {
		return await this.db.collection('opendb-admin-tableType').where({
	...
	async list(param) {
		...
		let {
			total
		} = await this.db.collection('opendb-admin-tableType').where(match).count();
		let {
			data
		} = await this.db.collection('opendb-admin-tableType')
			.where(match)
			.orderBy('sort', "asc")
			.skip((page - 1) * size)
			.limit(size)
